...
| public | renderMergeFields  | container:HTMLElement => void                                       | Renders a `div` with a `label` with the find property and an `input` with the replace property at the target container for each merge field in the source template.                                                                                                                                                                                                                        |
| public | renderSourceFields | container:HTMLElement => void                                       | Renders a `div` with a `label` with the placeholder value, an `input` with the current value and a file uploading `input` at the target container for each source field with a placeholder value in the source template.                                                                                                                                                                   |
